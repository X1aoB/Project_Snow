...
class MiaExpressionRuntimeTests(TestCase):
    ...
    def test_manifest_records_approval_and_explicit_unverified_waiver(self) -> None:
        self.assertEqual(
            ...
            "project-snow-mia-expression-runtime-1",
        )
    ...
    def test_all_runtime_assets_are_lossless_content_hashed_face_crops(self) -> None:
        self.assertEqual(self.manifest["transform"]["crop_box_xyxy"], [440, 145, 635, 340])
        self.assertEqual(self.manifest["transform"]["format"], "lossless_webp")
        for state, record in self.manifest["expressions"].items():
            ...
            relative_asset = record["asset_path"].removeprefix("/")
            asset_path = PUBLIC_ROOT / relative_asset
            self.assertTrue(asset_path.is_file(), state)
            digest = sha256(asset_path)
            self.assertEqual(record["asset_sha256"], digest, state)
            self.assertRegex(
                asset_path.name,
                rf"^{re.escape(state)}\.{digest[:16]}\.webp$",
                state,
            )
            with Image.open(asset_path) as image:
                self.assertEqual(image.format, "WEBP", state)
                self.assertEqual(image.size, (384, 384), state)
                self.assertEqual(image.mode, "RGBA", state)

    def test_client_maps_every_asset_and_limits_switching_to_mia_stage(self) -> None:
        for state, record in self.manifest["expressions"].items():
            self.assertIn(
                f'{state}: "{record["asset_path"]}"',
                self.javascript,
                state,
            )
        ...
        self.assertIn("function fillStagePortrait(node, character, message = null)", self.javascript)
        self.assertIn(
    ...
    def test_public_materials_page_discloses_the_waiver_and_manifest(self) -> None:
        self.assertIn("米娅舞台表情（0.9.4）", self.privacy_html)
