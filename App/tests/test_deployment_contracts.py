...
"""
    ...
    def test_prepare_script_installs_host_tuning_and_deploy_key(self) -> None:
        ...
        self.assertIn("AllowUsers deploy root", script)
        self.assertIn("Root public-key SSH requires one safe non-empty", script)
        ...
        self.assertIn("Refusing SSH hardening", script)
    ...
    def test_deploy_account_has_only_a_root_owned_release_runner(self) -> None:
        ...
        self.assertIn("root_authorized_keys_safe", runner)
        self.assertIn("permitrootlogin without-password", runner)
        self.assertIn("sshd_allowed_users_are_exact", runner)
        self.assertIn("'deploy root '", runner)
...
"""
    ...
    def test_ci_and_local_validation_include_direct_origin_assets(self) -> None:
        ...
        self.assertIn("PUBLIC_MAILER_ENV_FILE: /tmp/project-snow-mailer.env", workflow)
        self.assertIn("install -m 600 /dev/null /tmp/project-snow-mailer.env", workflow)
        ...
        self.assertIn("$publicEnvFixture = New-TemporaryFile", validate_all)
        self.assertIn("$mailerEnvFixture = New-TemporaryFile", validate_all)
        self.assertIn("$env:PUBLIC_MAILER_ENV_FILE = $mailerEnvFixture.FullName", validate_all)
        self.assertIn("Remove-Item -LiteralPath $publicEnvFixture.FullName -Force", validate_all)
        self.assertIn("Remove-Item -LiteralPath $mailerEnvFixture.FullName -Force", validate_all)
...
"""
    ...
    def test_public_frontend_build_fingerprints_immutable_assets(self) -> None:
        ...
        self.assertIn("COPY .build/public-ui/public_frontend ./public_frontend", dockerfile)
        ...
        self.assertIn('href="/app.css?v=0.10.0-rc.3"', public_html)
        self.assertIn('src="/app.js?v=0.10.0-rc.3"', public_html)
        ...
        self.assertIn("PUBLIC_APP_VERSION=0.10.0-rc.3", public_env)
