...
class DeploymentContractTests(TestCase):
    ...
    def test_deploy_stages_only_the_inactive_colour(self) -> None:
        ...
        self.assertLess(loopback_smoke, stage_env)
        ...
        self.assertIn("Staged API does not have the exact loopback binding", script)
        self.assertIn('.HostIp == "127.0.0.1"', script)
        self.assertIn("NetworkSettings.Ports can be empty", script)
        self.assertNotIn('.NetworkSettings.Ports["8000/tcp"][0].HostIp', script)
        self.assertIn("Active API was not healthy before Docker firewall reconciliation", script)
        self.assertIn("Active API was not healthy after Docker firewall reconciliation", script)
        self.assertIn("wait_for_active_container 15", script)
        self.assertIn("candidate_reconnect_attempt", script)
        self.assertIn("label=com.docker.compose.project=project-snow-public", script)
        self.assertIn("UFW must be active before Docker firewall reconciliation", script)
        self.assertIn("install_docker_ufw_ordering || exit 73", script)
        self.assertIn("20-project-snow-after-ufw.conf", script)
        self.assertIn("Wants=ufw.service", script)
        self.assertIn("<<'PROJECT_SNOW_DOCKER_UFW_ORDER' || return 1", script)
        self.assertIn('mv -f -- "$docker_ufw_dropin_tmp" "$docker_ufw_dropin" || return 1', script)
        self.assertIn("systemctl daemon-reload || return 1", script)
        ordering_install = script.index("install_docker_ufw_ordering || exit 73")
        self.assertIn("stat -c %u:%g:%a:%h", script)
        self.assertNotIn('if [ ! -e "$docker_reconcile_marker" ]', script)
        binding_gate = script.index("if ! candidate_binding_is_exact")
        failed_loopback = script.index("if ! wait_for_candidate_loopback")
        conditional_restart = script.index("systemctl restart docker", failed_loopback)
        self.assertLess(ordering_install, conditional_restart)
        self.assertLess(binding_gate, failed_loopback)
        self.assertLess(failed_loopback, conditional_restart)
        self.assertIn("port_bindings: .HostConfig.PortBindings", script)
