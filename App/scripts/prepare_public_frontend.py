...
def _git(repo: Path, *arguments: str, data: bytes | None = None) -> bytes:
    result = subprocess.run(["git", "-c", "core.autocrlf=false", *arguments], cwd=repo,
                            input=data, capture_output=True, timeout=90, check=False)
...
def _path(value: str) -> bool:
    ...
    return bool(value and not path.is_absolute() and path.as_posix() == value
                and "\\" not in value and not any(part in {"", ".", ".."} for part in value.split("/")))
...
def _export(repo: Path, commit: str, destination: Path, paths: tuple[str, ...]) -> None:
    ...
    stream = io.BytesIO(_git(repo, "cat-file", "--batch", data="".join(oid + "\n" for _, oid in entries).encode()))
...
def _patch(repo: Path, commit: str, workspace: Path, manifest_name: str, *, delta: bool) -> dict:
    ...
    _require(isinstance(filename, str) and bool(re.fullmatch(r"public-0\.9\.6(?:-r1)?\.patch", filename)), "invalid compatibility patch name")
    ...
    if delta:
        ...
        _require(bool(re.fullmatch(r"[0-9a-f]{40}", manifest.get("source_fix_commit", ""))), "invalid delta source commit")
    else:
        _require(manifest.get("baseline_commit") == BASELINE and _sha(payload) == ORIGINAL_PATCH_SHA, "immutable baseline/patch mismatch")
    before_key, after_key = ("before_sha256", "after_sha256") if delta else ("baseline_sha256", "compatible_sha256")
    files = manifest.get("files", [])
    _require(bool(files) and len({item["path"] for item in files}) == len(files), "invalid compatibility file list")
    for item in files:
        path = item["path"]
        _require(_path(path) and (path.startswith("App/public_frontend/") or path == "App/" + FINGERPRINT), "patch path outside allowed scope")
        target = workspace / path
        expected = item[before_key]
        _require((not target.exists()) if expected is None else target.is_file() and _sha(target.read_bytes()) == expected, "compatibility input hash mismatch")
    ...
    for item in files:
        _require(_sha((workspace / item["path"]).read_bytes()) == item[after_key], "compatibility output hash mismatch")
    return {"manifest_sha256": _sha(_blob(repo, commit, "App/compat/" + manifest_name)),
            "patch_sha256": _sha(payload), **({"source_fix_commit": manifest["source_fix_commit"]} if delta else {"baseline_commit": BASELINE})}
...
def _inventory(root: Path) -> list[dict]:
    ...
    for relative in STATIC_ROOTS:
        ...
        _require(not any(parent.is_symlink() for parent in directory.parents if parent != root and parent.is_relative_to(root)), "symlinked static ancestor")
        ...
        for path in sorted(directory.rglob("*")):
            ...
            files.append({"path": path.relative_to(root).as_posix(), "size": len(payload), "sha256": _sha(payload)})
...
def _identity(value: dict) -> None:
    ...
    _require(value["schema_version"] == SCHEMA and value["track"] in {"compat", "current"}, "invalid identity schema/track")
    _require(isinstance(value["version"], str) and bool(re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]{0,63}", value["version"])), "invalid frontend version")
    _require(isinstance(value["bundle_sha256"], str) and bool(re.fullmatch(r"[0-9a-f]{64}", value["bundle_sha256"])), "invalid bundle digest")
...
def verify_bundle(output: Path, *, expected_source_commit: str | None = None) -> dict:
    ...
    for name in (IDENTITY_FILE, MANIFEST_FILE):
        _require((output / name).is_file() and not (output / name).is_symlink(), "missing or symlinked bundle metadata")
    identity = _json((output / IDENTITY_FILE).read_bytes())
    _identity(identity)
    manifest = _json((output / MANIFEST_FILE).read_bytes())
    _require(set(manifest) == {"schema_version", "source_commit", "edition", "inputs", "identity", "files"}, "invalid bundle manifest fields")
    _require(manifest["schema_version"] == BUNDLE_SCHEMA and manifest["identity"] == identity, "bundle metadata identity mismatch")
    _require(bool(re.fullmatch(r"[0-9a-f]{40}", manifest.get("source_commit", ""))), "invalid source commit")
    if expected_source_commit is not None:
        _require(manifest["source_commit"] == expected_source_commit, "bundle belongs to another source commit")
    _require(manifest["edition"] == ("compat-096" if identity["track"] == "compat" else "current"), "edition/track mismatch")
    inputs = manifest["inputs"]
    compat = identity["track"] == "compat"
    _require(isinstance(inputs, dict) and set(inputs) == ({"static_commit", "selector_sha256", "compatibility", "delta"} if compat else {"static_commit", "selector_sha256"}), "invalid bundle provenance fields")
    _require(inputs["static_commit"] == (BASELINE if compat else manifest["source_commit"]), "static source commit mismatch")
    _require(isinstance(inputs["selector_sha256"], str) and bool(re.fullmatch(r"[0-9a-f]{64}", inputs["selector_sha256"])), "invalid selector digest")
    if compat:
        for key, source_key in (("compatibility", "baseline_commit"), ("delta", "source_fix_commit")):
            item = inputs[key]
            _require(isinstance(item, dict) and set(item) == {"manifest_sha256", "patch_sha256", source_key}, "invalid patch provenance fields")
            _require(all(isinstance(item[field], str) and bool(re.fullmatch(r"[0-9a-f]{64}", item[field])) for field in ("manifest_sha256", "patch_sha256")), "invalid patch provenance digest")
            _require(isinstance(item[source_key], str) and bool(re.fullmatch(r"[0-9a-f]{40}", item[source_key])), "invalid patch source commit")
        _require(inputs["compatibility"]["baseline_commit"] == BASELINE and inputs["compatibility"]["patch_sha256"] == ORIGINAL_PATCH_SHA, "original compatibility provenance mismatch")
...
def prepare(app_root: Path, output: Path) -> dict:
    ...
    _require(set(config) == {"schema_version", "edition", "compat_version", "current_version"}, "invalid UI selector fields")
    _require(config["schema_version"] == "project-snow-public-frontend-release-1" and config["edition"] in {"compat-096", "current"}, "invalid UI selector")
    compat = config["edition"] == "compat-096"
    version = config["compat_version"] if compat else config["current_version"]
    identity = {"schema_version": SCHEMA, "track": "compat" if compat else "current", "version": version, "bundle_sha256": "0" * 64}
    _identity(identity)
    ...
    with TemporaryDirectory(prefix=".public-ui-build-", dir=output.parent) as temporary:
        ...
        _export(repo, static_commit, workspace, tuple("App/" + path for path in STATIC_ROOTS) + ("App/" + FINGERPRINT,))
        inputs = {"static_commit": static_commit, "selector_sha256": _sha(_blob(repo, source_commit, "App/" + SELECTOR))}
        if compat:
            inputs["compatibility"] = _patch(repo, source_commit, workspace, "public-0.9.6.manifest.json", delta=False)
            ...
            inputs["delta"] = _patch(repo, source_commit, workspace, "public-0.9.6-r1.manifest.json", delta=True)
        selected = workspace / "App"
        subprocess.run([sys.executable, "-I", str(selected / FINGERPRINT), "--app-root", str(selected)],
        ...
        manifest = {"schema_version": BUNDLE_SCHEMA, "source_commit": source_commit, "edition": config["edition"],
                    "inputs": inputs, "identity": identity, "files": files}
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, help="default: <app-root>/.build/public-ui; must not exist when preparing")
    parser.add_argument("--verify", action="store_true", help="verify copied bundle without rebuilding/Git")
    parser.add_argument("--expected-source-commit", help="verify provenance; defaults to nonempty APP_REVISION")
    args = parser.parse_args()
    output = args.output or args.app_root / ".build/public-ui"
    try:
        result = verify_bundle(output, expected_source_commit=args.expected_source_commit or os.getenv("APP_REVISION") or None) if args.verify else prepare(args.app_root, output)
