...
from .dialogue_output import (
    _communication_block_violations,
    _normalize_content_blocks,
    _normalize_content_blocks_with_diagnostics,
...
from .output_validation import (
    _clean_renderable_text,
    _decode_json_candidate,
    _parse_model_json,
)
from .output_validation import (
    _extract_partial_model_answer as _extract_partial_model_answer,
)
from .output_validation import (
    _looks_like_model_envelope as _looks_like_model_envelope,
)
from .output_validation import (
    _looks_like_structured_fragment as _looks_like_structured_fragment,
)
...
_SESSION_STATES = ScopedStateMap("snow_sessions")
_WORLD_STATES = ScopedStateMap("snow_worlds")
...
class MVPService:
    ...
    @staticmethod
    @contextmanager
    def isolated_state(session_id: str, world_id: str, session: dict[str, Any], world: WorldState):
        with _SESSION_STATES.isolate({session_id: session}), _WORLD_STATES.isolate({world_id: world.values}):
            yield
    ...
    def _post_model(
        ...
        **kwargs: Any,
        ...
        budget = active_generation_budget()
        if budget is None and max_provider_http_calls is not None:
            budget = GenerationBudget(max_provider_http_calls, current_calls)
        if budget is not None:
            try:
                budget.consume()
            except GenerationBudgetExceeded as exc:
                raise MVPProviderError("本次操作已达到模型请求次数上限。") from exc
...
"""
    ...
    # Compatibility delegates; content rules live in the pure output module.
    _normalize_content_blocks_with_diagnostics = staticmethod(_normalize_content_blocks_with_diagnostics)
    _normalize_content_blocks = staticmethod(_normalize_content_blocks)
    _render_content_blocks = staticmethod(_render_content_blocks)
    _communication_block_violations = staticmethod(_communication_block_violations)
