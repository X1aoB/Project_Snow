...
The local deployment entrypoint fetches `origin/main`, refuses a SHA that is not an ancestor of that ref, checks out the exact commit as detached HEAD, verifies it, and only then enters `App/ops/deploy.sh`. This prevents a caller from deploying an unmerged feature-branch commit or running deployment scripts from a different checkout than the selected image SHA.
