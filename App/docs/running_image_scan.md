...
临时 `/tmp` 保留 512 MiB 上限。`TMPDIR=/cache/tmp` 指向本次 mode 0700 私有缓存内的磁盘目录，
用于漏洞库下载和解包，避免 Java 数据库压缩包占满内存临时目录；内存上限不会因此扩大。
扫描器只能读导出的镜像，写私有临时缓存/报告目录。
数据库下载阶段使用默认 bridge 访问官方数据库仓库；实际镜像分析阶段关闭网络。
数据包识别采用 offline 模式，数据库在本次运行中重新下载。漏洞库及 Java 数据库是扫描必需、
会随漏洞披露而变化的工具数据；下载不会更新扫描器镜像或业务镜像。
它不是对运行容器可写层、宿主机或配置秘密的扫描。

总 deadline 包含发现、导出、漏洞库/Java 数据库下载与扫描，默认 30 分钟，允许 1–60 分钟。
失败或超时最多另用两个 10 秒清理调用，且只删除本工具 UUID 标签和实际 ID 同时匹配的扫描容器。
不存在全局 prune，也不会清理业务容器。默认每镜像最多 4 GiB。宿主及缓存文件系统必须保留
至少 10 GiB 空闲，每个数据库下载前另需至少 4 GiB 临时空间，即缓存至少空闲 14 GiB；
逐镜像导出前另需 `max(2 GiB, 2 × image.Size + 1 GiB)` 余量。
下载、导出与扫描结束后还会复查 10 GiB 保留空间，不足时失败而不记录扫描通过。
