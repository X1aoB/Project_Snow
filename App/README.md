...
`backend.snow_app.public_main` is a separate registration-free application for `snow.xiaob.dev`. Its public web/API release is **1.0.0**, with 22-character text and in-person conversations, local history and anonymous feedback. This release improves expression/dialogue timing, adds brief expression crossfades, prepares character images before entering the stage, and enlarges text-chat portraits with better desktop and mobile spacing. Day continuity is a settings preference that defaults to starting today, without a prompt on every character switch. See [the 1.0.0 release notes](docs/release-1.0.0.md).

Personal Codex subscription connections now sit alongside the existing API Key flow. Each user logs in locally, pairs their own connector, and explicitly saves an account-supported model and reasoning effort; Luna Max is the preferred default when available. Saved choices stay in the current tab, uncertain requests are not automatically resent, and subscription connections pause automatic summaries while preserving the user's API-mode preference. [The connector guide](docs/codex-subscription-connector.md) covers setup and subscription limits.

The public service retains `/public/v1`, signed `public-state-2` packages and IndexedDB v4. Stable assistant `displayBlocks` and valid stage cues remain stored with messages so segmented replies survive switching and reload without replaying historical animation. API Key credentials use fixed, non-renewing 12-hour AES-GCM envelopes bound to an anonymous HttpOnly cookie; both API and subscription browser credentials stay in the current tab's `sessionStorage`. The internal `/api/v1` workspace is not mounted.
