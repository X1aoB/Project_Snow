...
`snow.xiaob.dev` 的独立公开沉浸式客户端当前为 `0.9.3`：它不挂载内部工作台，聊天记录保存在浏览器
IndexedDB，BYOK 密钥只在当前标签页的 `sessionStorage` 中保存加密 envelope，固定最长 12 小时且不滑动
续期。生成时，密钥和当前请求仍会经 Project Snow 代理发送给用户选择的模型供应商；关闭标签页会清除
...
| 当前版本 | 本地应用 `v0.5.0`；公开沉浸式客户端 `0.9.3` |
